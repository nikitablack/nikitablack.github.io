...
Finally I'm at the leaf node, no more unknown variables! And I already forgot why do I need all this... Ah, I wanted to create `Shader Modules`. To recal here's the dependency chain: `VkShaderModule` -> `VkDevice` -> `VkPhysicalDevice` -> `VkInstance`. And here's how I done this in code:
...
In remainig part of this lesson I'll do exatly this.