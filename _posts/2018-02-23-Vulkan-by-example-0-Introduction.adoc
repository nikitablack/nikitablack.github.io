...
I rewrote the tutorials multiple times trying to find the best style. In the end I came to conclusion that I don't want to copy the code to the article, but leave a link to a file on `Github` - this saves a lot of space and does not reuire to update the article in case the code changes. If you find this not very comfortable to read -buy a second monitor- tell me and we'll try to come up with something to solve the problem.
