...
The `addImpl()` function accepts a `std::function` as a second parameter. In different `add()` functions I call it and pass a callable object which will be converted to `std::function` implicitly.
