...
Next we're mapping the system memory to the upload buffer (*#8*) and creating out first command with https://msdn.microsoft.com/en-us/library/windows/desktop/dn903856(v=vs.85).aspx[`ID3D12GraphicsCommandList::CopyBufferRegion()`] (*#9*). It will gpu to copy `bufferSize` amount of data from upload buffer (which actually points to the system memory) to default buffer.
