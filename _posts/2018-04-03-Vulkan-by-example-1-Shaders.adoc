...
MaybeQueueFamilies get_device_graphics_and_present_queue_families(VkPhysicalDevice const physicalDevice, VkSurfaceKHR const surface)
{
	...
	if (graphicsQueueIndex == -1)
		return tl::make_unexpected("failed to find graphics queue"); // #d
	
	int presentQueueIndex{-1};
	for (size_t i{0}; i < queueFamilies.size(); ++i) // #e
	{
	...
	if (presentQueueIndex == -1)
		return tl::make_unexpected("failed to find present queue"); // #f
	
	return make_tuple(static_cast<uint32_t>(graphicsQueueIndex), static_cast<uint32_t>(presentQueueIndex)); // #g
}
----
...
I bet you already can recognize this pattern with closed eyes.
