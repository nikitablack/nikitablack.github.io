...
video::8RCWeKLlVew[youtube, width=640, height=360, float="left"]
...
image::https://raw.githubusercontent.com/nikitablack/articles_stuff/master/vulkan_by_example/lesson_0/teapot_initial_patches.png["1", 400, align="center"]
...
To fix this I have to tweak indices a bit and still use mirror matrix. In the end I store all the data in `TeapotData.h` file - 28 patches in total. For visualisation I also added a vector with colors. Later in shader with patch id (and I know this id - the runtime kindly provides it) I will sample transforms and colors and apply the changes for corresponding patch.
