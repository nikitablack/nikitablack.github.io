...
When we have a list filled with commands we need to tell the gpu that we want it to do some work. We can do it with `ID3D12CommandQueue` interface. It should be the same type as our list and allocator.