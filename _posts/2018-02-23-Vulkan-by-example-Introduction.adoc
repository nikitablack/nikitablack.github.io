...
`AppData`, as I already told, is a state holder - it holds all the objects required by the application. In this tutorial it's pretty empty:
...
All I know for now is that I need to render the `teapotData` onto some surface - a `window`. Also as I speak about tesselation I need a way to control that tesselation and I do it with `tessLevel`.
...
Then set a callback with http://www.glfw.org/docs/latest/group__input.html#ga7e496507126f35ea72f01b2e6ef6d155[`glfwSetKeyCallback`] which listens for `+`, `-` or `space` keys to change tesselation level or rendering mode.
...
In this callback I need to access the current state of the application (`AppData`) so I pass an instance to `glfw` with `glfwSetWindowUserPointer()`.
