...
Next we're calling https://msdn.microsoft.com/en-us/library/windows/desktop/dn899178(v=vs.85).aspx[`ID3D12Device::CreateCommittedResource()`] method that actually reserves a memory (*#3*). This method asks gpu to find the free space. There're other methods for resource creation - for example we can use already reserved memory and create placed resource in it - just like a `placement new` operator in `c++` (we'll not use this in our demo).
