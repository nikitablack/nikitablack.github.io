...
After initializaition `MainApplication::run()` which loops until the window is closed:
...
----
Here in the loop I call `glfw` callbacks and do rendering (empty in this lesson).
...
----
For now I just destroy a window.

===== Conclusion
Wow, the whole post and no any single `Vulkan` related line of code! Sorry, that was a long indroduction and I didn't tell about a lot of things I wanted but I promise the next lesson will be much more interesting.