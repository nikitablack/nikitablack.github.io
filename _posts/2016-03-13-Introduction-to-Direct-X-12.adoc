...
NOTE: One more interesting thing. Most of directX 12 methods return `HRESULT` that is `S_OK` in case of operation success and error code in case of failure. We can check the result with macro https://msdn.microsoft.com/en-us/library/windows/desktop/ms693474%28v=vs.85%29.aspx[`FAILED`] or [`SUCCEEDED`](https://msdn.microsoft.com/en-us/library/windows/desktop/ms687197%28v=vs.85%29.aspx).