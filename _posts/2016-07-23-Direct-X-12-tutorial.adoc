...
Next is a pure math - in the `main()` function we have a list of control points (16 points) and we need to sample them so we can assign a position to the newly generated by tesselator vertex. The good overview of the math you can find http://www.gamasutra.com/view/feature/131755/curved_surfaces_using_bzier_.php[here]. Also http://www.gdcvault.com/play/1012740/direct3d[here] is a good presentation about tesselation in directx 11 (I took the shader code from there to be honest). Also worth to read https://msdn.microsoft.com/en-us/library/windows/desktop/ff476340(v=vs.85).aspx[directx 11 tesselation overview] if you're not familar with this stage at all.
