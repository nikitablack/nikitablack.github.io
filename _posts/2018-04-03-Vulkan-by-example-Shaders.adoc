...
To summarize: here's the dependency chain: `VkShaderModule` -> `VkDevice` -> `VkPhysicalDevice` -> `VkSurfaceKHR` -> `VkInstance`. And here's how this chain managed in the code:
