...
===== Preparations
The basic code flow is pretty straightforward: in `main` we prepare `Vulkan` objects and start an _infinite_ loop. Of course it's not infinite - it will be stopped as soon as window is closed. After that happens everything should be cleared, we don't want to keep the garbage after our party, isn't it?

`AppData`, as I already told, is a state holder - it holds all the required objects. In this first tutorial it's pretty empty:
