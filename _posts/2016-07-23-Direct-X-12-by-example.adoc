...
We're describing the view with the strut `D3D12_SHADER_RESOURCE_VIEW_DESC`. Since we can have arbitrary stride in structured buffer the formad defined as `DXGI_FORMAT_UNKNOWN`. All other parameters are pretty strightforward