...
If the proper GPU was found we continue to the next step - we need to know the format of the surface we are going to render to. A given device and a surface can support multiple formats and we choose one in 
`get_device_surface_format` helper function. So first we get all supported formats. Next we check for the special case.  Here's a quote from the specification:
