...
Looks scary. But going step by step we'll get familar with a lot of `directx12` concepts. In the top we're creating different description structures where we're declaring that we're going to create default heap and a buffer. Notice that we didn't specify the purpose of the buffer - we just declared the size. In other words we asked for a certain amount of memory.
...
If we'll look back on `device->CreateCommittedResource()` we'll see that we for the `4th` parameter we used `D3D12_RESOURCE_STATE_COPY_DEST` for the default buffer and `D3D12_RESOURCE_STATE_GENERIC_READ` for the upload. These are initial states of our resources. In order to manipulate memory it should be in some state and if it's not we'll get an error. For upload buffer documentation states that it should be created with `D3D12_RESOURCE_STATE_GENERIC_READ` state. And in order to copy from the source to destination the destination should be in `D3D12_RESOURCE_STATE_COPY_DEST`.

[TODO: insert about update]

After we finished with an update we need to transition default buffer to the state we provided as a `createDefaultBuffer()` function parameter. For different resources this state should be different. For example for constand or vertex buffer it should be `D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER`, for structured buffer - `D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE`. So we creating a command that tells gpu to transition to necessary state.

Finally we tell the gpu to execute our commands with `commandQueue->ExecuteCommandLists()` method. But before we need to close the command list or we'll get an error. If we'll exit the method now we'll get an undefined behavior. When we tell queue to execute a list the gpu doesn't start to do it immediately. Instead the commands are queued and nobody knows when they'll be finished. That mean that if we leave the method now the upload buffer will be destroyed (we're not keeping pointer to it) and when the gpu will be ready to execute a copy command the source will not be valid any more. Let;s fix this.

Previously we talked about cpu-gpu communication. Now we're interesting in gpu-cpu talk. We can do it with fences. Fence is nothing more than an integer value. After we submitted a command list we can add one more command to the queue that will set fence to specified value. All we left to do is to check wherether our fence have a correct value or not and if not just wait untill it change. Super simple, isn't it? First we're creating `ID3D12Fence` itself and also some `fenceEventHandle`. This is not a part of `directx` but `winapi`. We're setting fence on the gpu with `commandQueue->Signal()` method. The first parameter is a fence object and the second is a desired value we want out fence be after command list executed. Next we're setting an event on completion with `fence->SetEventOnCompletion()` method. When the fence value will be equal the first parameter the event (second parameter) should be raised. Next with `WaitForSingleObject()` we're waiting when this happen for specified number of seconds (`10` seconds in our case). If on moment of calling this function the fence already have desired value it will return `WAIT_OBJECT_0` immediately.