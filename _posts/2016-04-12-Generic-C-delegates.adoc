...
You don't need to know an assembly to find the interesting thing - the call to `call2()` was completely optimized out. In other words - using the template wrapper will be replaced by the compiler to the actual callback invokation! It's hard to tell will the compiler optimize the same way in real big project but at least it gives some sort of hope that it will.
...
* member functions - the members of class or struct. The pointer to this function should be called on valid object
