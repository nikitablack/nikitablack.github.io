...
The https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/main.cpp[main.cpp] file starts with global variables initialization. Globals are bad, you say. Indeed, I'm not a fan as well. But here's the deal: `glfw` library, which I use for window manipulation, uses callbacks for communication. For example, `glfwSetKeyCallback` calls user provided function on a keyboard press. The callback defined in https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/window/on_key_press.cpp[on_key_press.cpp] file. As you see, there's no way to pass `AppDataPtr` to that function. It is still possible to pass data around with `glfwSetWindowUserPointer` and `glfwGetWindowUserPointer`, but that breaks the agreement of a _single-unique-pointer_. So I decided to go with global variables. For now, there are https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/Global.h[three]

The next thing the `main` does is defines `appData` variable which is immidiatelly passed to https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/window/create_window.cpp[create_window] function. Here the standard `glfw` stuff happens, if you want to know more, please refer to the https://www.glfw.org/docs/latest/quick.html[documentation]. Next, the `appData` updated with a newly created window. Finally, two callbacks are setted up - the https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/window/on_key_press.cpp[on_key_press] you already know - on `+` press the tesselation level increaes, on `-` - decreases, `space` press changes the rendering mode. Here the aforementioned flobal variables gets updated.

NOTE: The maximum tesselation level for your GPU can be found in an instance of `VkPhysicalDeviceLimits`. It's too early to know about it right now, but what is important to know is that the specification guarantees certain values limits for certain GPU properties. `maxTessellationGenerationLevel`, for example, is guaranteed to be in the range 0 to 64.

https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/window/framebuffer_size_callback.cpp[framebuffer_size_callback] will be called on window resize.

Back to https://github.com/nikitablack/vulkan_by_example_2/blob/lesson_0/src/teapot/src/main.cpp[main] - after the window creation (if the creation didn't fail) the _infinite_ loop is started. Of course it's not _that_ infinite - it will be stopped as soon as window is closed. After that happens everything should be cleared - we don't want to leave the garbage after the party, right?

===== Conclusion
Well, the whole post and no any single `Vulkan` related line of code! Sorry for that, it was a necessary long indroduction but I promise - in the next lesson we'll start the real work.
