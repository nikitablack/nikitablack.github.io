...
* global functions - the functions declared globally and static functions. The pointer to such a function we will pass as template to our delegate. For example `&globalFunction` will return the address of a function (`&` is optional). And we can be sure we will not have problems, because this functions always exist during program lifetime.
