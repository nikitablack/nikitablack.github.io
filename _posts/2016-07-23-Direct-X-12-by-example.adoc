...
Now when we have updated constant buffer we're ready to finish with the root signature. We're setting the last parameter - root descriptor (*#13*) as a first parameter not forgetting the correct offset from descriptor heap start. 